...
## [<img src="https://github.com/QuantLet/Styleguide-and-FAQ/blob/master/pictures/qloqo.png" alt="Visit QuantNet">](http://quantlet.de/) **PyEx5** [<img src="https://github.com/QuantLet/Styleguide-and-FAQ/blob/master/pictures/QN2.png" width="60" alt="Visit QuantNet 2.0">](http://quantlet.de/)

```yaml

Name of QuantLet : PyEx5

Published in : SPL

Description : Solutions to Exercises Day 5

Keywords : 'functions, recursive, loops, regression'
...
###############################################################################
#  5. quartiles di?erence ˜ x0.75 - ˜ x0.25 
##############################################################################
...
#   Y i = X2 i + 0.5ei, ei ~ N(0,1), i = 1,...,100 Xi ~ U[-1,1]. 
...
#   recession-velocity = H ·distance, check if H is di?erent from zero. 
#   Plot rec.vel and the ?tted values against the distance.
###############################################################################
...
#   Estimate linear dependence ratingi = ß1sugarsi +ß2fati +ei, 
#   check if all ß’s are di?erent from zero and if they all together are signi?cant. 
#   Plot the data points and the ?tted values in 3D scatterplot.
###############################################################################
...
###############################################################################
#6. Simulate 500 samples from a GARCH(1,1) model with (?,a1,ß1) = (0.3,0.4,0.36)
#   , e ~ N(0,1). Plot the time series. Plot the ACF and PACF of the time series 
#   and the squared time series. Fit GARCH(1,1), GARCH(2,1) and GARCH(1,2) to 
#   the simulated time series. Compare the ACF, PACF of the residuals of the three ?ttings.
###############################################################################
np.random.seed(100)
...
for i in range(1, n):
    sigsq[i] = ? + a1*(eps[i-1]**2) + b1*sigsq[i-1]
...
###############################################################################
#7. Download the S&P500 data from Moodle, and ?t a proper time series model for that. 
#   • Plot the time series 
#   • Take log-di?erence, plot it. See if there are volatility clustering. 
